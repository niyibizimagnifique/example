...
function App() {
  ...
  useEffect(() => {
    const checkLoginStatus = async () => {
      ...
      if (isAuthenticated) {
        ...
        if (storedStudents && storedCourses && storedDepartments) {
  ...
  const fetchDepartments = async () => {
    try {
      ...
      localStorage.setItem('departments', JSON.stringify(departmentsList)); // Store fetched departments in local storage
    } catch (error) {
  ...
  const addDepartment = async () => {
    try {
      ...
      const existingIdDepartment = departments.find(dep => dep.id === newDepartment.id);
      const existingNameDepartment = departments.find(dep => dep.name === newDepartment.name);
      ...
      await example_backend.addDepartment(newDepartment.id, newDepartment.name);
  ...
  return (
    <main>
      ...
      <h1>Students, Courses, and Departments Management (Example)</h1>
